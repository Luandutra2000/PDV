...
const serviceWorkerJs = await readFile(new URL('../service-worker.js?v=20260804-05', import.meta.url), 'utf8');
const appJs = await readFile(new URL('../src/app.js?v=20260804-05', import.meta.url), 'utf8');
const mobileDashboardJs = await readFile(new URL('../src/modules/mobile/mobile-dashboard.module.js?v=20260804-05', import.meta.url), 'utf8');
...
assert(indexHtml.includes('./src/app.js?v=20260804-05'), 'app entrypoint should use the latest cache-busting version');
...
assert(serviceWorkerJs.includes('pdv-v78'), 'service worker cache name should change when app modules change');
assert(serviceWorkerJs.includes('./src/app.js?v=20260804-05'), 'service worker should precache the latest app entrypoint');
assert(appJs.includes('./utils/dom.js?v=20260804-05'), 'shared DOM helper import should be versioned with the module graph');
assert(appJs.includes('./modules/vendas/vendas.module.js?v=20260804-05'), 'critical UI modules should be versioned with the module graph');
...
assert(appJs.includes('./modules/mobile/mobile-dashboard.module.js?v=20260804-05'), 'mobile dashboard import should use the latest cache-busting version');
assert(mobileDashboardJs.includes('../../services/mobile-notifications.service.js?v=20260804-05'), 'mobile feed service import should use the latest cache-busting version');
