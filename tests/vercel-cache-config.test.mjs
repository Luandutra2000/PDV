...
const serviceWorkerJs = await readFile(new URL('../service-worker.js?v=20260804-04', import.meta.url), 'utf8');
const appJs = await readFile(new URL('../src/app.js?v=20260804-04', import.meta.url), 'utf8');
const mobileDashboardJs = await readFile(new URL('../src/modules/mobile/mobile-dashboard.module.js?v=20260804-04', import.meta.url), 'utf8');
...
assert(indexHtml.includes('./src/app.js?v=20260804-04'), 'app entrypoint should use the latest cache-busting version');
...
assert(serviceWorkerJs.includes('pdv-v77'), 'service worker cache name should change when app modules change');
assert(serviceWorkerJs.includes('./src/app.js?v=20260804-04'), 'service worker should precache the latest app entrypoint');
assert(appJs.includes('./utils/dom.js?v=20260804-04'), 'shared DOM helper import should be versioned with the module graph');
assert(appJs.includes('./modules/vendas/vendas.module.js?v=20260804-04'), 'critical UI modules should be versioned with the module graph');
...
assert(appJs.includes('./modules/mobile/mobile-dashboard.module.js?v=20260804-04'), 'mobile dashboard import should use the latest cache-busting version');
assert(mobileDashboardJs.includes('../../services/mobile-notifications.service.js?v=20260804-04'), 'mobile feed service import should use the latest cache-busting version');
